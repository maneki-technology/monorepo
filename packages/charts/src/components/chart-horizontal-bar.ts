/**
 * <chart-horizontal-bar> — Horizontal bar chart Web Component.
 *
 * Same as chart-bar but with swapped axes — categories on Y, values on X.
 *
 * Usage (declarative):
 *   <chart-horizontal-bar
 *     title="Top Languages"
 *     labels='["JS","Python","TS","Java","Go"]'
 *     datasets='[{"label":"Usage %","data":[25,20,15,10,8]}]'
 *   ></chart-horizontal-bar>
 */
...
const OBSERVED_ATTRS = [
  ...
  "show-tooltips", "bar-gap", "group-gap", "description",
] as const;
...
class ChartHorizontalBarElement extends HTMLElement {
  ...
  private _syncFromAttributes(): void {
    ...
    this._options = {
      ...
      barGap: numAttr(this, "bar-gap") ?? this._options.barGap,
      groupGap: numAttr(this, "group-gap") ?? this._options.groupGap,
    };
  }
  ...
  private _render(): void {
    ...
    titleSvg.textContent = opts.title ?? "Horizontal Bar Chart";
    ...
    if (!this.hasAttribute("aria-label")) {
      this.setAttribute("aria-label", opts.title ?? "Horizontal Bar Chart");
    }
    ...
    // Layout — wider left padding for category labels
    const leftPad = 120;
    const rightPad = 40;
    const bottomPad = 60;
    ...
    // Scales: X = value axis, Y = category axis
    const { min: dataMin, max: dataMax } = dataExtent(datasets);
    const xScale = linearScale(dataMin, dataMax, plotW, opts.xAxis);
    const yScale = categoryScale(opts.labels, plotH, 0.1);

    // Grid: vertical lines for value ticks
    ...
    // Y-axis labels (left — category labels)
    ...
    const groupHeight = bandHeight * (1 - groupGap);
    const barHeight = groupHeight / numDatasets;
    const innerBarHeight = barHeight * (1 - barGap);
    const barOffset = (barHeight - innerBarHeight) / 2;
    const zeroX = plotX + xScale.scale(0);
    ...
    for (let di = 0; di < numDatasets; di++) {
      ...
      for (let ci = 0; ci < ds.data.length; ci++) {
        ...
        const categoryY = plotY + yScale.start(ci);
        const groupStart = categoryY + (bandHeight - groupHeight) / 2;
        const barY = groupStart + di * barHeight + barOffset;

        const valueX = plotX + xScale.scale(value);
        ...
        rect.setAttribute("height", String(innerBarHeight));
...
customElements.define("chart-horizontal-bar", ChartHorizontalBarElement);
export { ChartHorizontalBarElement };
