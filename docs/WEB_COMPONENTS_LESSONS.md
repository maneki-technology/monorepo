...
## Shadow DOM Blocks Pointer Event Propagation for Drag-and-Drop

When using `grid-layout` with Shadow DOM content (e.g., `ui-card` inside `grid-item`), `closest("grid-item")` can't cross shadow boundaries. The event target is inside the shadow root, so DOM traversal stops at the shadow boundary.

**Fix:** Walk `composedPath()` to find the `grid-item` ancestor:

```ts
const path = e.composedPath();
const gridItem = path.find(el => el instanceof HTMLElement && el.tagName === "GRID-ITEM");
```

Also, `setPointerCapture` must be called on the `grid-layout` element itself, not the event target. If captured on a shadow DOM child, pointer events route to the wrong element. Use `pointermove`/`pointerup` listeners on `this` (the grid-layout) instead of `document`.

## Lit Re-renders Fight Drag State

When Lit's `@state()` triggers re-render during drag, it resets grid positions. The render cycle recreates DOM elements, losing the in-progress drag coordinates.

**Fix:** Use `repeat()` directive with stable keys so Lit reuses existing DOM nodes. Cache layout state outside of reactive properties. Don't reassign `@state()` arrays in layout-change handlers — mutate in-place instead to avoid triggering a full re-render.

## pointer-events: none on Shadow DOM Children

Setting `pointer-events: none` on a Shadow DOM component (e.g., `ui-card`) prevents all events inside it, including slotted content. This is because the Shadow DOM host intercepts the pointer-events CSS — children can't opt back in.

**Fix:** Overlay elements that need clicks must be siblings outside the component, not children inside it. For drag handles, place them as siblings of the Shadow DOM component rather than inside its slot.

## Build Order Matters for .d.ts Files

`tsc && vite build` wipes `.d.ts` files because Vite's `emptyOutDir` clears `dist/` before writing JS output. The TypeScript declarations generated by `tsc` are deleted before the build completes.

**Fix:** Always use `vite build && tsc --emitDeclarationOnly`. Vite builds JS first (clearing `dist/`), then tsc adds `.d.ts` files on top.

## setRangeText for Undo-Safe Text Insertion
