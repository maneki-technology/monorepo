...
import type { Album, Tag } from "./gallery-types.js";
...
export class GalleryUploadWizard extends LitElement {
  ...
  private async _createQuickAlbum() {
    ...
    try {
      ...
      if (res.ok) {
        // Re-fetch albums from parent
        this.dispatchEvent(new CustomEvent("albums-changed", { bubbles: true, composed: true }));
        // Wait a tick for parent to update albums prop
